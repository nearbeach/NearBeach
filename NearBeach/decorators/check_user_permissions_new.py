...
def check_user_customer_permissions(min_permission_level):
    ...
    def decorator(func):
        ...
        def inner(request, *args, **kwargs):
            ...
            passes, user_level = object_customer(request)
...
def check_user_organisation_permissions(min_permission_level):
    ...
    def decorator(func):
        ...
        def inner(request, *args, **kwargs):
            ...
            _, user_level = object_organisation(request)
...
def check_user_permissions(min_permission_level, object_lookup=""):
    ...
    def decorator(func):
        ...
        def inner(request, *args, **kwargs):
            ...
            # Override object lookup
            destination = object_lookup
            if object_lookup == "":
                destination = kwargs["destination"]

            # Depending on the destination - depends on the function we use to look up
            # User permissions
            ## TO DO - finish this code.

            passes, user_level = object_generic(request, destination, kwargs)
...
# Internal function
def object_change_task(request, kwargs):
...
# Internal Function
def object_generic(request, object_lookup, kwargs):
    # Default user level is 0
    user_group_results = UserGroup.objects.filter(
        is_deleted=False,
        username=request.user,
...
# Internal Function
def object_kanban_board(request, kanban_card_id):
    # Default user level is 0
    user_group_results = UserGroup.objects.filter(
        is_deleted=False,
        username=request.user,
...
# Internal Function
def object_organisation(request):
    # Default user level is 0
    user_group_results = UserGroup.objects.filter(
        is_deleted=False,
        username=request.user,
...
# Internal Function
def object_requirement_item(request, kwargs):
