...
describe("Shakeout Group -> as Admin", () => {
	...
	it("Click on the 'Add Group' button and start adding a new group", () => {
		...
		cy.get("#errorModalContent").should(
			"contain",
		...
		let group_name = `A Unique Named Group ${Math.random()
			.toString()
			.substr(2, 8)}`;
		cy.get(".col-md-8 > :nth-child(1) > .form-control")
			.clear()
		...
		cy.get(".col-md-8 > :nth-child(1) > .form-control").should(
			"have.value",
			group_name
		);

		cy.get(".col-md-8 > :nth-child(2) > .v-select").should(
			"contain",
			"QA Team"
		);
	});
