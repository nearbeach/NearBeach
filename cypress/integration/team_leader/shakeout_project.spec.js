...
describe("Shakeout Project", () => {
	...
	it("New Project Test", () => {
		...
		cy.get(".col-md-12 > .btn").type("Create New Project");
		...
		cy.get(".modal-footer > .btn")
			...
			.click()
			.click(); //Click done twice due to bug
		...
		cy.get(".form-group > .form-control").type(project_title);
		...
		cy.get(
			":nth-child(2) > .form-group > .vdatetime > .vdatetime-input"
		).click();
		...
		cy.get(
			":nth-child(3) > .form-group > .vdatetime > .vdatetime-input"
		).click();
		cy.get(".vdatetime-popup__year").click();
		cy.get(":nth-child(100)").click();
		cy.get(".vdatetime-popup__actions__button--confirm").click();
		...
		//Get the group lists
		cy.get("#vs2__combobox > .vs__selected-options > .vs__search").type(
			"QA Team{enter}Admin{enter}"
		);
	...
	it("Read the current project", () => {
		...
		cy.get(
			":nth-child(3) > .col-md-8 > .form-group > .form-control"
		).should("have.value", project_title);

		//Check to make sure the project status is NEW
		//cy.get('.vs__selected')
		cy.get("#vs1__combobox").contains("New");
		// .should('have.text', '\n New\n           ');

		// cy.get('.organisation_name')
		cy.get(".organisation-name > a").contains("NearBeach Organisation");
