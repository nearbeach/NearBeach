<template>
	<div v-if="allowedObjects.includes(destination)">
		...
		<div v-if="currentSprints.length === 0"
...
export default {
	...
	data() {
		return {
			...
			currentSprints: [],
			importVariables: {
				header: "List of Child Sprints",
				prefix: "sprint",
				id: "sprint_id",
				title: "sprint_name",
				status: "sprint_status",
			},
			sprintNameModel: "Sprint Name",
			sprintStartDate: 0,
			sprintEndDate: 0,
		}
	},
	...
	methods: {
		...
		getSprints() {
			...
			this.axios.post(
				...
				this.currentSprints = response.data;

				//Fix up the models
				const data_length = response.data.length + 1;
				...
				this.sprintNameModel = `${this.destination}-${this.locationId} - Sprint ${data_length} - ${date}`;
			}).catch((error) => {
				this.$store.dispatch("newToast", {
