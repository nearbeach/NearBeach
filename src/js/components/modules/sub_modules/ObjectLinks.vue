		...
		methods: {
			extractObjectDescription(link) {
				...
				if (
					...
					link_title =
						link.requirement_item_id__requirement_item_title;
			...
			extractObjectStatus(link) {
				...
				if (
					...
					link.requirement_id !== null &&
					this.destination !== "requirement"
				) {
					// The link is a requirement link
					link_status =
						link.requirement_id__requirement_status__requirement_status;
				} else if (
					link.requirement_item_id !== null &&
					this.destination !== "requirement_item"
				) {
					// The link is a requirement item link
					link_status =
						link.requirement_item_id__requirement_item_status__requirement_item_status;
				} else if (
					link.task_id !== null &&
					this.destination !== "task"
				) {
				...
				axios
					...
					.then((response) => {
                         ...
                         */
							...
							var sum =
								parseInt(0 + row.project_id) +
								parseInt(0 + row.requirement_id) +
								parseInt(0 + row.requirement_item_id) +
								parseInt(0 + row.task_id) +
